...
The header-stack patch is integrated and stable: announcement and header remain standalone sections inside a shared wrapper (`[data-yaomri-header-group]`) and the stack-level hide/show controls are owned there. The repo is generally coherent and launchable for internal QA, but there are still clear areas to strengthen before a sellable CNVRT-facing release.
...
1. Global loading model
   - `layout/theme.liquid` loads `critical.css` (preload) and `yaomri-base.css` globally.
...
2. Global vs section load map
   ...
   - `sections/header.liquid`: `yaomri-header.css`, `mega-menu.css`, `mega-menu.js`, `yaomri-header-state.js`
