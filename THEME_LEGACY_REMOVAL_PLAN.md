...
|---|---|---|---|---|---|---|---|---|---|
| `yaomri-` | Yes (header, announcement, hero, mega, cart, stack wrapper) | Yes (global component styles) | Yes (`yaomri-header-state.js`, `mega-menu.js`, fallbacks) | Yes | Partial | No | High | Keep as compatibility alias; remove per-component only after selector parity pass | C |
| `ysh-` | Yes (Single Image Hero inline vars) | Yes (`section-single-image-hero.css` fallbacks) | No direct JS | Yes (`--cnvrt-single-hero-*`) | N/A | No | Medium | Keep until hero CSS drops `--ysh-*` fallbacks | B |
| `yh-` | Yes (Header inline vars) | Yes (`yaomri-header.css` fallbacks/state vars) | Indirect via CSS var writes | Yes (`--cnvrt-header-*`) | N/A | No | High | Keep until header/state var chain is fully CNVRT-only | C |
| `yab-` | Yes (Announcement inline vars) | Yes (`announcement-bar.css` fallbacks) | No | Yes (`--cnvrt-announcement-*`) | N/A | No | Medium | Keep until announcement CSS no longer consumes `--yab-*` fallbacks | B |
| `ym-` | Yes (Mega inline vars) | Yes (`mega-menu.css` fallbacks) | Yes (`mega-menu.js` writes `--ym-*`) | Yes (`--cnvrt-mega-*`) | Partial | No | High | Keep; JS/CSS both still write/read `--ym-*` | C |
...
| `mdrawer-` | Yes (drawer classes + data attrs + html lock class) | Yes (`mobile-drawer.css`) | Yes (`mobile-drawer.js`, header-state lock check) | Yes | Partial | No | High | Keep; open/close lock and accessibility flows still depend on legacy hooks | C |
...
| `icon-yaomri` | Yes (icon snippets + renders) | Yes (icon sizing/fill selectors) | No | No full icon namespace replacement | N/A | No | Medium | Defer as dedicated icon-system migration | E |
...
### 3) Single Image Hero
- Liquid: CNVRT single-hero classes are now primary in markup; legacy `--ysh-*` compatibility vars remain emitted.
...
### 6) Cart
- Liquid: dual `yaomri-cart*` + `cnvrt-cart*` classes.
...
### 7) Mobile Drawer
- Liquid: dual classes and dual data attrs (`data-mdrawer-*` + `data-cnvrt-drawer-*`).
- CSS: html lock and structure styles still include `mdrawer-open`/legacy selectors.
- JS: uses both namespaces; toggles both `mdrawer-open` and `cnvrt-drawer-open`.
- CNVRT equivalent: yes.
...
### 8) Mega Menu
- Liquid/snippets: dual `yaomri-mega*` + `cnvrt-mega*`.
- CSS: mixed legacy/CNVRT selector groups; legacy vars still used as fallbacks.
- JS: uses both class namespaces; writes both `--cnvrt-mega-*` and `--ym-*` runtime vars.
- CNVRT equivalent: yes.
- JS prefers CNVRT: partial.
- Remove legacy markup classes now: **No**.
- Remove legacy CSS aliases now: **No**.
- Keep old vars: **Yes** (`--ym-*`).
- Risk: **High**.
- Action: keep until mega JS stops writing legacy var names and selector parity is validated.
- Class: **C**.

### 9) Header / Header Stack
- Liquid/layout: dual classes and data attrs (`data-yaomri-header-group` + `data-cnvrt-header-group`), legacy + CNVRT stack vars.
...
- Remove legacy markup classes now: **No**.
- Remove legacy CSS aliases now: **No**.
- Keep old vars: **Yes** (`--yh-*`, `--yaomri-header-stack-*`).
- Risk: **High**.
- Action: keep until a dedicated header-state refactor retires legacy vars and selectors together.
- Class: **C**.

### 10) Icon system
- Legacy icon namespace (`icon-yaomri*`) is still rendered by snippets and consumed by CSS.
...
6. Single Image Hero markup-only `yaomri-single-hero*` aliases in `sections/single-image-hero.liquid` were safe and have now been removed (Phase 5 complete).
...
1. `icon-yaomri*` snippet/class namespace and references.
...
- Scope:
  ...
  11. Removed `yaomri-single-hero*` classes from `sections/single-image-hero.liquid`.
  12. Kept single-image-hero CSS selector fallbacks and `--ysh-*` variable fallbacks for one release.
...
- Remove `yaomri-cart*` classes from markup.
- Keep all `cnvrt-cart*` classes.
- Do not edit `assets/yaomri-cart.css` or `assets/yaomri-cart.js` in this phase.
